...
@Component({
  ...
  styleUrl: './existence.component.scss'
})
...
constructor(private service:ServiceService<any>) {
 ...
 getData(){
   ...
   this.service.Data("existence/index").subscribe({
     next:(n)=>{
 ...
 applyFilter(event: Event) {
   ...
   let url ="existence/create";

   if (this.action) {
     url =  "existence/update"
   }
     this.service.Post(url,this.MyForm.value).subscribe({
       ...
       this.MyForm.reset()
         ...
         this.Toast.fire({
           position: 'top-end',
           icon: 'success',
           title: `Se ha ${url =='existence/create'?'insertado':'actualizado'} correctamente`,
         });
       },
       error:(e)=>{
         this.action = false
         ...
         this.Toast.fire({
           position: 'top-end',
           icon: 'error',
           title: ` No se ha podido ${url =='existence/create'?'insertar':'actualizar'} correctamente`,
         });
       }
     ...
     deleterow(row:any){
       this.isLoading = true
       this.service.Delete(`existence/destroy/${row.id}`).subscribe({
         next:(n)=>{
           this.Toast.fire({
             position: 'top-end',
             icon: 'success',
