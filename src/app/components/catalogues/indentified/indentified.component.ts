...
@Component({
  ...
  templateUrl: './indentified.component.html',
  styleUrl: './indentified.component.scss'
})
export class IndentifiedComponent {
...
constructor(private service:ServiceService<any>) {
 ...
 getData(){
   this.service.Data("indentified/index").subscribe({
     next:(n)=>{
 ...
 applyFilter(event: Event) {
   ...
   let url ="indentified/create";

   if (this.action) {
     url =  "indentified/update"
   }
     this.service.Post(url,this.MyForm.value).subscribe({
       ...
       this.MyForm.reset()
         ...
         this.Toast.fire({
           position: 'top-end',
           icon: 'success',
           title: `Se ha ${url =='indentified/create'?'insertado':'actualizado'} correctamente`,
         });
       },
       error:(e)=>{
         this.action = false
         ...
         this.Toast.fire({
           position: 'top-end',
           icon: 'error',
           title: ` No se ha podido ${url =='indentified/create'?'insertar':'actualizar'} correctamente`,
         });
       }
     ...
     deleterow(row:any){
       this.isLoading = true
       this.service.Delete(`indentified/destroy/${row.id}`).subscribe({
         next:(n)=>{
           this.Toast.fire({
             position: 'top-end',
             icon: 'success',
