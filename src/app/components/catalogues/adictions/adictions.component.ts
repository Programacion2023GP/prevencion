...
@Component({
  ...
  templateUrl: './adictions.component.html',
  styleUrl: './adictions.component.scss'
})
export class AdictionsComponent {
...
constructor(private service:ServiceService<any>) {
 ...
 getData(){
   ...
   this.service.Data("adictions/index").subscribe({
     next:(n)=>{
 ...
 applyFilter(event: Event) {
   ...
   let url ="adictions/create";

   if (this.action) {
     url =  "adictions/update"
   }
     this.service.Post(url,this.MyForm.value).subscribe({
       ...
       this.MyForm.reset()
         ...
         this.Toast.fire({
           position: 'top-end',
           icon: 'success',
           title: `Se ha ${url =='adictions/create'?'insertado':'actualizado'} correctamente`,
         });
       },
       error:(e)=>{
         this.action = false
         ...
         this.Toast.fire({
           position: 'top-end',
           icon: 'error',
           title: ` No se ha podido ${url =='adictions/create'?'insertar':'actualizar'} correctamente`,
         });
       }
     ...
     deleterow(row:any){
       this.isLoading = true
       this.service.Delete(`adictions/destroy/${row.id}`).subscribe({
         next:(n)=>{
           this.Toast.fire({
             position: 'top-end',
             icon: 'success',
