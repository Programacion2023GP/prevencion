...
@Component({
  ...
  styleUrl: './dependences.component.scss'
})
export class DependencesComponent {
...
constructor(private service:ServiceService<any>) {
 ...
 applyFilter(event: Event) {
   ...
   let url ="dependence/create";

   if (this.action) {
     url =  "dependence/update"
   }
     this.service.Post(url,this.MyForm.value).subscribe({
       ...
       this.MyForm.reset()
         ...
         this.Toast.fire({
           position: 'top-end',
           icon: 'success',
           title: `Se ha ${url =='dependence/create'?'insertado':'actualizado'} correctamente`,
         });
       },
       error:(e)=>{
         this.action = false
         ...
         this.Toast.fire({
           position: 'top-end',
           icon: 'error',
           title: ` No se ha podido ${url =='dependence/create'?'insertar':'actualizar'} correctamente`,
         });
       }
     ...
     deleterow(row:any){
       this.isLoading = true
       this.service.Delete(`dependence/destroy/${row.id}`).subscribe({
         next:(n)=>{
           this.Toast.fire({
             position: 'top-end',
             icon: 'success',
