...
@Component({
  ...
  styleUrl: './activies.component.scss'
})
export class ActiviesComponent {
  ...
  constructor(private service:ServiceService<any>) {
   ...
   applyFilter(event: Event) {
     ...
     if (this.action) {
       ...
       this.service.Post(url,this.MyForm.value).subscribe({
         ...
         this.MyForm.reset()
           ...
           this.Toast.fire({
             position: 'top-end',
             icon: 'success',
             title: `Se ha ${url =='activies/create'?'insertado':'actualizado'} correctamente`,
           });
         },
         error:(e)=>{
           this.action = false
           ...
           this.Toast.fire({
             position: 'top-end',
             icon: 'error',
             title: ` No se ha podido ${url =='activies/create'?'insertar':'actualizar'} correctamente`,
           });
         }
       ...
       deleterow(row:any){
         this.isLoading = true
         this.service.Delete(`activies/destroy/${row.id}`).subscribe({
           next:(n)=>{
             this.Toast.fire({
               position: 'top-end',
               icon: 'success',
