...
@Component({
  ...
  styleUrl: './childrens.component.scss'
})
...
constructor(private service:ServiceService<any>) {
 ...
 getData(){
   this.service.Data("childrens/index").subscribe({
     next:(n)=>{
 ...
 applyFilter(event: Event) {
   ...
   let url ="childrens/create";

   if (this.action) {
     url =  "childrens/update"
   }
     this.service.Post(url,this.MyForm.value).subscribe({
       ...
       this.MyForm.reset()
         ...
         this.Toast.fire({
           position: 'top-end',
           icon: 'success',
           title: `Se ha ${url =='childrens/create'?'insertado':'actualizado'} correctamente`,
         });
       },
       error:(e)=>{
         this.action = false
         ...
         this.Toast.fire({
           position: 'top-end',
           icon: 'error',
           title: ` No se ha podido ${url =='childrens/create'?'insertar':'actualizar'} correctamente`,
         });
       }
     ...
     deleterow(row:any){
       this.isLoading = true
       this.service.Delete(`childrens/destroy/${row.id}`).subscribe({
         next:(n)=>{
           this.Toast.fire({
             position: 'top-end',
             icon: 'success',
